...
class GoogleCloud(Cloud):
    ...
    K8S_VERSION = "latest"
    K8S_ALPHA_VERSION = "1.13.4-gke.10"
    ZONE = "us-central1-a"
    PROJECT = "myproject"
    IMAGE_TYPE = "COS"
    DISK_TYPE_ON_NODE = "pd-ssd"

    def __init__(self):
        super(GoogleCloud, self).__init__()
        self.instance_type = GoogleCloud.INSTANCE_TYPE
        self.k8s_version = GoogleCloud.K8S_VERSION
        self.k8s_alpha_version = GoogleCloud.K8S_ALPHA_VERSION
        self.zone = GoogleCloud.ZONE
        self.project = self.env.get("GKE_PROJECT_ID", GoogleCloud.PROJECT)
        self.user = self.env.get("GCE_USER", GoogleCloud.CLOUD_USER)
        self.image_type = GoogleCloud.IMAGE_TYPE
        self.disk_type_on_node = GoogleCloud.DISK_TYPE_ON_NODE
        self.enabled = True
    ...
    def build_cloud(self):
        ...
        self.disks = self.prompts.prompt_integer("Enter number of local SSD disks for MapR FS. Each disk will be a fixed 375GB", self.disks, 1, key_name="GKE_NUM_DISKS")
        self.instance_type = self.prompts.prompt("GKE compute instance type?", self.instance_type, key_name="GKE_INSTANCE_TYPE")
        if self.alpha:
        ...
        Log.info("Using GKE compute image type: {0}".format(self.image_type), True)
        if self.alpha:
            Log.info("Using alpha Kubernetes version", True)
        else:
            Log.info("Using non-alpha Kubernetes version", True)

        if not self.prompts.prompt_boolean("Ready to create Google GKS cluster. Do you want to continue?", True, key_name="GKE_CREATE"):
    ...
    def create_k8s_cluster(self):
        Log.info("Creating cluster with {0} nodes of type {1} with {2} local ssd(s) of size 375GB...".
