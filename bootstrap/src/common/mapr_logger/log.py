...
class Log(object):
    ...
    def get_console_level():
        if Log._console_logger is None or \
                len(Log._console_logger.handlers) == 0:
    ...
    def _log(level, msg, args, exc_info=None, extra=None):
        ...
        if Log._console_logger is not None:
            # remove tracebacks from being shown in the console for all levels
            exc_info = None

            # add a visible message to the console if there was a error
            level_to_prefix = {
                logging.WARNING: "WARNING",
                logging.ERROR: "ERROR",
                logging.CRITICAL: "CRITICAL"
            }
            if level_to_prefix.get(level):
                msg = "{}: {}".format(level_to_prefix.get(level), str(msg))
            record = Log._console_logger.makeRecord(Log._console_logger.name,
                                                    level, fn, lno, msg, args,
                                                    exc_info, func, extra)
            Log._console_logger.handle(record)
