...
if [[ ! -d "${VIRTUALENV}" ]]; then
    ...
    if [ $? -eq 0 ]; then
      ...
      echo "ERROR: Could not determine Python version; Will assume Python 2"
      PYTHON_VER=2
    fi

    if [ $PYTHON_VER -ne 2 ] && [ $PYTHON_VER -ne 3 ]; then
      echo "ERROR: Python version ${PYTHON_VER} is not compatible"
      exit 2
    fi
    ...
    if [ $? -eq 0 ]; then
      ...
      echo "Python 3 venv module is not available"
      VENV_AVAIL=2
    fi
    ...
    if [ $PYTHON_VER -eq 2 ]; then
      VE=$(command -v "virtualenv")
      if [[ $? -ne 0 ]]; then
          echo "ERROR: In order to create a Python 2 virtual environment you need the virtualenv package. Please run 'pip install virtualenv'"
          exit 3
      fi
      VE_OPT=""
    fi
